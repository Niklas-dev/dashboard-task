          ...
          {data.length >= 1 ? (
            data.map((transaction) => (
              ...
              >
                <th
                  scope="row"
                  className="px-6 py-4 font-medium  whitespace-nowrap  capitalize rounded-l-xl border-2 border-r-0 border-gray-200"
                >
                  <p>{transaction.transaction.name}</p>
                  <p>{transaction.transaction.id}</p>
                </th>
                <td className="px-6 py-4 capitalize border-t-2 text-lg border-b-2 border-gray-200">
                  {transaction.amount} NOK
                </td>
                <td className="px-6 py-4 border-t-2 border-b-2 text-lg border-gray-200 md:block hidden">
                  {transaction.date}
                </td>
                <td className="px-6 py-4 border-l-0 border-2 text-lg border-gray-200 rounded-r-xl">
                  <div className="bg-red-600 text-red-800 font-medium text-lg bg-opacity-20 text-center rounded-lg h-9 px-3 flex justify-center items-center">
                    {transaction.status}
                  </div>
                </td>
              </tr>
